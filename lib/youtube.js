  ...
  return new Promise(async (resolve, reject) => {
    ytdl
      ...
      .then(async (getUrl) => {
        ...
        let tinyUrl = (await link(resultFix[0].video)).result;
        let title = getUrl.videoDetails.title;const ytdl = require("ytdl-core");
const yts = require("yt-search");
const axios = require("axios");
  ...
  return new Promise((resolve, reject) => {
    ytdl
      ...
      .then(async (getUrl) => {
        ...
        let tinyUrl = (await link(resultFix[0].audio)).result;
        let title = getUrl.videoDetails.title;
        let desc = getUrl.videoDetails.description;
        let views = parseInt(getUrl.videoDetails.viewCount || 0);
        let likes = getUrl.videoDetails.likes;
        let dislikes = getUrl.videoDetails.dislikes;
        let channel = getUrl.videoDetails.ownerChannelName;
        let uploadDate = getUrl.videoDetails.uploadDate;
